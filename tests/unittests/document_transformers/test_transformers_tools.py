...
from langchain_rag.document_transformers import CopyDocumentTransformer, \
    GenerateQuestionsTransformer, SummarizeTransformer
from langchain_rag.document_transformers.sumarize_and_questions_transformer import \
    SummarizeAndQuestionsTransformer
from tests.unittests.fake_llm import FakeLLM
...
async def test_copy_transformer_alazy_transform_documents() -> None:
    doc1 = Document(page_content="my test")
    doc2 = Document(page_content="other test")
    result = [doc async for doc in
              CopyDocumentTransformer().alazy_transform_documents(
                  iter([doc1, doc2]))]
    assert len(result) == 2
    assert id(result[0]) != id(doc1)
    assert id(result[1]) != id(doc2)
...
def test_generate_questions_transform_documents() -> None:
    ...
    llm = init_llm({
        ...
        1: "\n\n"
           ...
           "2. What are some of the mathematical methods and notation used in the past?\n"
...
async def test_generate_questions_alazy_transform_documents() -> None:
    ...
    transformer = GenerateQuestionsTransformer.from_llm(llm=llm)
    result = [doc async for doc in
              transformer.alazy_transform_documents(
                  iter([doc1, doc2]))]
    assert len(result) == 6
...
def test_sumarize_transformer_transform_documents() -> None:
    ...
    """)
    llm = init_llm({
        0: 'SUMMARY:\nMathematics is the study of numbers, formulas, shapes, spaces, '
           'quantities, and their changes.',
        1: 'SUMMARY:\nMathematics has a long history of discoveries and the development '
...
def test_sumarize_and_questions_transformer_transform_documents() -> None:
    ...
    llm = init_llm({
        0: 'Output:\n'
           '```\n'
           '{\n'
           '    "summary": "Mathematics is an area of knowledge that includes topics such as numbers, formulas, shapes, spaces, quantities, and their changes.",\n'
           '    "questions": [\n'
        ...
        1: 'Output:\n'
           '    ```\n'
           '    {\n'
           '        "summary": "The history of mathematics deals with the origin of discoveries in mathematics and the mathematical methods and notation of the past.",\n'
           '        "questions": [\n'
